...
const routes = [
...

  {
    ...
    handler: async function(req, res) {
      ...
      _.each(payload, product => {
        let command =
          "INSERT INTO products(name, cost) VALUES ($1,$2)";
        let values = [product.name, product.cost];
        promises.push(
          db
            .any(command, values)
            .then(data => {
  ...
  },
  {
    ...
    handler: async function(req, res) {
      ...
      let promises = [];
       _.each(payload, product => {
      let command = "update products set cost= $2 where name=$1";
      let values = [product.name, product.cost];
  ...
  },
  {
    ...
    handler: async function(req, res) {
      ...
      let promises = [];
       _.each(name, product => {
      let command = "select cost from products where name='"+product+"'";
      promises.push(
         db
           .any(command)
           .then(data => {
  ...
  },
  {
    ...
    handler: async function(req, res) {
      ...
      let promises = [];
       _.each(payload, product => {
      let command = "delete from products where name=$1";
      let values = [product.name];
      console.log(values,command);
      promises.push(
         db
           .any(command, values)
